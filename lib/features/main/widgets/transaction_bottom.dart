...
class _AddTransactionBottomSheetState
  ...
  /// Returns the correct category list based on the selected transaction type.
  /// When Expense is selected → expense categories only.
  /// When Income is selected → income categories only.
  List<AppCategory> get _currentCategories {
    return _selectedType == 0
        ? CategoryHelpers.expenseCategories
        : CategoryHelpers.incomeCategories;
  }

  /// Ensures _selectedCategory is always valid for the current type.
  void _resetCategoryForType() {
    final categories = _currentCategories;
    final exists = categories.any((c) => c.name == _selectedCategory);
    if (!exists) {
  ...
  void initState() {
    ...
    if (widget.transactionToEdit != null) {
      ...
      // Canonicalise the stored category name to a known value
      final canonical = CategoryHelpers.canonicalise(
      ...
      _selectedCategory = CategoryHelpers.expenseCategories.first.name;
    }
  }
  ...
  void _confirm() async {
    ...
    // Ensure a valid category is selected from the correct type list
    _resetCategoryForType();
    final categoryName = _selectedCategory ?? _currentCategories.first.name;

    String? categoryId;
    if (isExpense) {
      // Only expense transactions get a category_id (per existing schema)
    ...
    final dataMap = {
      ...
      'description': isExpense
          ...
          : (_descriptionController.text.trim().isEmpty
              ? categoryName
              : '$categoryName | ${_descriptionController.text.trim()}'),
    ...
    // Refresh all data providers in parallel.
  ...
  Widget build(BuildContext context) {
    ...
    // Get the filtered category list for the currently selected type
    final categories = _currentCategories;
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Center(
              child: CustomSlidingSegmentedControl<int>(
                ...
                onValueChanged: (v) => setState(() {
                  _selectedType = v;
                  // Reset to first category of the new type when toggling
            ...
            // ── Category chips ────────────────────────────────────────────
            // The list automatically switches between expense and income
            // categories when the toggle changes.
